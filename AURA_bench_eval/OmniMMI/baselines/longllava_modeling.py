...
from longllava.conversation import conv_templates
from longllava.model.builder import load_pretrained_model
from longllava.mm_utils import tokenizer_image_token, get_model_name_from_path, process_images
...
class Args:
    def __init__(self):
        ...
        self.model_dir = ''
        # Device to use for computation (e.g., 'cuda:0')
        self.device = 'cuda:0'
        # The length of the subImage for image patching
        self.patchside_length = 336
        # Strategy to apply for patching. Options include "norm" and "bestFit".
        # "Norm" means resize the Image, "bestFit" means patch the image into subImages with patchside_length*patchside_length
        self.patchStrategy = 'norm'
        # Whether the input is a video
        self.isVideo = False
        # The maximum number of frames for a video
        self.frameNum = 128        
        # Extract frame every t seconds for video
        self.t = 1.0


class LongLLaVA(ViLLMBaseModel):
    def __init__(self, model_args):
        ...
        self.args = Args()
        self.args.model_dir = self.model_path
        self.args.device = f"cuda:{self.device}"

        self.bot = Chatbot(self.args)
    ...
    def generate(self, instruction, video_path):
        ...
        outputs = self.bot.chat(images=[video_path], text=instruction, isVideo=True, t=self.args.t, frameNum=self.args.frameNum)
