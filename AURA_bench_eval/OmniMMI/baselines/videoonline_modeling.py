...
from videoonline import build_model_and_tokenizer, parse_args, fast_greedy_generate
...
class VideoOnline:
    def __init__(self, model_args) -> None:
        ...
        args.resume_from_checkpoint = model_path
        args.llm_pretrained = "checkpoints/Meta-Llama-3-8B-Instruct"
        args.vision_pretrained = "checkpoints/siglip-large-patch16-384"
    ...
    def input_video_stream(self, video_time):
        ...
        if frame_idx > self.last_frame_idx:
            ...
            # video_tensor = self.video_tensor[ranger]
    ...
    def load_video(self, video_path):
        ...
        # # HACK modify video to given fps bound by max frames
        # self.video_tensor, _, fps = read_video(video_path, pts_unit='sec', output_format='TCHW')
        # self.num_video_frames = self.video_tensor.size(0)
        # fps = fps["video_fps"]
        # duration = self.num_video_frames / fps
        # max_frame = 128
        # if self.num_video_frames > max_frame:
        #     self.frame_fps = max_frame / duration
        #     num_frames = int(duration * self.frame_fps)
        #     step_size = max(1, self.num_video_frames // num_frames)
        #     self.video_tensor = self.video_tensor[::step_size]
        #     self.num_video_frames = self.video_tensor.size(0)
        # self.video_tensor = self.video_tensor.to(self.device)
