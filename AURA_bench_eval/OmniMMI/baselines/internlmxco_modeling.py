...
from internlm_xcomposer2d5_ol_7b.base.ixc_utils import Video_transform
from internlm_xcomposer2d5_ol_7b.base.modeling_internlm_xcomposer2 import get_stopping_criteria
from internlm_xcomposer2d5_ol_7b.memory.constants import DEFAULT_IMAGE_PATCH_TOKEN
from internlm_xcomposer2d5_ol_7b.memory.grounding_qwen import GroundQwenForCausalLM
from internlm_xcomposer2d5_ol_7b.memory.constants import DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from internlm_xcomposer2d5_ol_7b.memory.mm_utils import tokenizer_image_token
...
class InternLMXCO(ViLLMBaseModel):
    def __init__(self, model_args):
        ...
        ixc_model_path = model_args["model_path"]
        ixc_tokenizer = AutoTokenizer.from_pretrained(f'{ixc_model_path}/merge_lora', trust_remote_code=True)
        ixc_model = AutoModelForCausalLM.from_pretrained(f'{ixc_model_path}/merge_lora', device_map=f"cuda:{device}", trust_remote_code=True).eval().cuda().to(torch.bfloat16)
        ixc_model.tokenizer = ixc_tokenizer

        kwargs = {"device_map": f'cuda:{device}'}
        kwargs['torch_dtype'] = torch.float16
        vs_tokenizer = AutoTokenizer.from_pretrained(f'{ixc_model_path}/memory', use_fast=False)
        vs_model = GroundQwenForCausalLM.from_pretrained(f'{ixc_model_path}/memory', low_cpu_mem_usage=True, **kwargs)
        ...
        self.vs_tokenizer = vs_tokenizer
        self.ixc_tokenizer = ixc_tokenizer
        self.ixc_model = ixc_model
    ...
    def generate(self, instruction, video_path, gen=True):
        ...
        seqs = preprocess_time(time_idx, num_clips, self.vs_tokenizer)
        seqs = torch.nn.utils.rnn.pad_sequence(
            seqs,
            batch_first=True,
            padding_value=self.vs_tokenizer.pad_token_id)
        compress_mask = seqs.ne(self.vs_tokenizer.pad_token_id)

        question = preprocess_question([question_], self.vs_tokenizer)
        question = torch.nn.utils.rnn.pad_sequence(
            question,
            batch_first=True,
            padding_value=self.vs_tokenizer.pad_token_id)
        qs_mask = question.ne(self.vs_tokenizer.pad_token_id)

        with torch.no_grad():
            with torch.inference_mode():
                with torch.cuda.amp.autocast():
                    similarity, glb, lol = self.vs_model.forward_grounding(
                        input_ids=seqs.to(device=self.device, non_blocking=True),
                        attention_mask=compress_mask.to(device=self.device, non_blocking=True),
                        images=video.to(dtype=torch.float16, device=self.device, non_blocking=True),
                        qs_ids=question.to(device=self.device, non_blocking=True),
        ...
        if len(sele_frames) > max_frame:
            step = (len(sele_frames) - 1) / (max_frame - 1)
            sele_frames = [sele_frames[int(i * step)] for i in range(max_frame)]
        img = img_process(sele_frames)

        mid_prompt = 'Question: ' + question
        # mid_prompt = f'''Is the following string an instruction? "{question}"''' # prompt for pt task
        query = f'[UNUSED_TOKEN_146]user\n{mid_prompt}[UNUSED_TOKEN_145]\n[UNUSED_TOKEN_146]assistant\nThe answer is'

        with torch.no_grad():
            with torch.cuda.amp.autocast():
                response = model_gen_withmem(self.ixc_model, query, img, glb, sele_lol, hd_num=36, do_sample=False, beam=1, max_new_token=1024)
            # print(response)        
