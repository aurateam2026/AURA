...
def get_model_output(model, video_processor, tokenizer, video, qs, args):
    ...
    # video_tensor = video_processor(video, fps=1, return_tensors='pt')['pixel_values'][0].half().to(args.device)
    ...
    if os.path.isdir(video):
        ...
        video = load_video(video, fps=1, max_frames=128)
    # video = load_video(video, fps=1)
    video_tensor = video_processor(video, return_tensors='pt')['pixel_values'].half().to(args.device)
