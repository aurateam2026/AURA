...
from .self_segment import segment, segment_left
...
class RMTRTransformerProjector(nn.Module):
    ...
    def forward(
        ...
        if t <= 1:
            ...
            # # if len(cls_states) == 2:
            # #     boundaries = [cls_states.shape[0]-1]
            # # else:
            # #     boundaries = segment(cls_states)
            # # boundaries = segment(cls_states)
            # # boundaries = segment(cls_states, k=3)
            # # boundaries = segment_left(cls_states)
            ...
            for s, bi in enumerate(boundaries):
                ...
                state_cache.append(last_hidden_states.clone())
                
                # trick: enc_memory
                enc_cache.append(last_hidden_states.clone())
            ...
            # # tricks
            # last_hidden_states = torch.cat(enc_cache, dim=1)
            ...
            # for param in self.projector.proj[0].parameters():
            #     print(param)
            # # print(self.projector.proj[0].parameters())
            
            # last_hidden_states = torch.cat(state_cache, dim=1)
