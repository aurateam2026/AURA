...
class TransformerProjector(nn.Module):
    def __init__(self, config, depth):
        ...
        self.read_memory_emb = nn.Parameter(torch.zeros(self.num_memory_tokens, config.hidden_size))
        self.memory_tokens = nn.Parameter(torch.randn(self.num_memory_tokens, config.hidden_size))
...
class RMTMLPTransformerProjector(nn.Module):
    def __init__(self, config, depth):
        super().__init__()
        self.config = config
        ...
        # self.abs_vid_pe = nn.Embedding(1024, config.hidden_size)
    ...
    def forward(
        ...
        if t <= 3:  #TODO: split image
            ...
            for s, bi in enumerate(boundaries):
                ...
                steps = torch.linspace(index, bi, min(16, bi-index+1), dtype=torch.int).to(hidden_states.device) # prevent from OOM
                # steps = torch.linspace(index, bi, bi-index+1, dtype=torch.int).to(hidden_states.device)
