...
from videollamb.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, VIDEO_TOKEN_INDEX, DEFAULT_VI_END_TOKEN, DEFAULT_VI_START_TOKEN
from videollamb.constants import IGNORE_INDEX, X_TOKEN_INDEX, X_INDEX_TOKEN, DEFAULT_X_PATCH_TOKEN, DEFAULT_X_START_TOKEN, DEFAULT_X_END_TOKEN, X_PLACEHOLDER


from videollamb.mm_utils import get_anyres_image_grid_shape
...
class LlavaMetaModel:
    ...
    def initialize_image_modules(self, model_args, fsdp=None):
        ...
        self.config.rt_num_attention_heads = 1
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, image_sizes):
        ...
        if type(images) is list or images.ndim == 5: # [B, 1, C, H, W] or [B, G, C, H, W]
            if type(images) is list:
                images = [x.unsqueeze(0) if x.ndim == 3 else x for x in images]
            concat_images = torch.cat([image for image in images], dim=0) # B*G, C, H, W
            # image_features = self.encode_images(concat_images) # B*G, num_patches, D
            ...
            image_features = self.get_model().mm_projector(image_features) # B G*P D
            
            # return image_features
            
            # split_sizes = [image.shape[0] for image in images]
            # image_features = torch.split(image_features, split_sizes, dim=0) # B, G, num_patches, D
            ...
            # raise ValueError("The images must be five dimension")
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        X, X_sizes=None, X_modalities=None, x_features=None,
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # CUR_X_TOKEN_INDEX = X_TOKEN_INDEX["IMAGE"]
